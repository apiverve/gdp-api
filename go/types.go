// Package gdp provides a Go client for the GDP Data API.
//
// For more information, visit: https://apiverve.com/marketplace/gdp?utm_source=go&utm_medium=readme
package gdp
...
// Request contains the parameters for the GDP Data API.
//
// Parameters:
//   - country (required): string - ISO 2 or 3-letter country code (e.g., US, USA, GB, GBR) [minLength: 2, maxLength: 3]
//   - year: integer - Specific year to retrieve data for (1960-present). Returns latest if not specified. [min: 1960, max: 2030]
type Request struct {
...
func (r *Request) Validate() error {
	rules := map[string]ValidationRule{
		"country": {Type: "string", Required: true, MinLength: intPtr(2), MaxLength: intPtr(3)},
		"year": {Type: "integer", Required: false, Min: float64Ptr(1960), Max: float64Ptr(2030)},
	}
...
// ResponseData contains the data returned by the GDP Data API.
type ResponseData struct {
	...
	Historical []HistoricalItem `json:"historical"`
}

// HistoricalItem represents an item in the Historical array.
type HistoricalItem struct {
	Year int `json:"year"`
	Gdp int `json:"gdp"`
	GdpFormatted string `json:"gdpFormatted"`
	GdpGrowth float64 `json:"gdpGrowth"`
	GdpPerCapita int `json:"gdpPerCapita"`
}
