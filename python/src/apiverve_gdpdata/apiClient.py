...
class GdpAPIClientError(Exception):
...
class ValidationError(GdpAPIClientError):
...
class GdpAPIClient:
    ...
    VALIDATION_RULES = {"country": {"type": "string", "required": True, "minLength": 2, "maxLength": 3}, "year": {"type": "integer", "required": False, "min": 1960, "max": 2030}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the GdpAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises GdpAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/gdp'
        self.headers = {
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[GdpAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns GDPDataData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises GdpAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise GdpAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise GdpAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise GdpAPIClientError(f"Request failed: {str(e)}")
