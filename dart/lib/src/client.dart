...
class GdpValidationException implements Exception {
  final List<String> errors;

  GdpValidationException(this.errors);

  @override
  String toString() => 'GdpValidationException: ${errors.join("; ")}';
}
...
/// GDP Data API client.
///
/// For more information, visit: https://apiverve.com/marketplace/gdp?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [country] (required) - ISO 2 or 3-letter country code (e.g., US, USA, GB, GBR) [minLength: 2, maxLength: 3]
/// * [year] - Specific year to retrieve data for (1960-present). Returns latest if not specified. [min: 1960, max: 2030]
class GdpClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'country': ValidationRule(type: 'string', required: true, minLength: 2, maxLength: 3),
    'year': ValidationRule(type: 'integer', required: false, min: 1960, max: 2030),
  };

  GdpClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/gdp',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [GdpValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw GdpValidationException(errors);
    }
  }

  /// Execute a request to the GDP Data API.
  ///
  /// Parameters are validated before sending the request.
  Future<GdpResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw GdpException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return GdpResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw GdpException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw GdpException('Resource not found');
      } else {
        throw GdpException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is GdpException) rethrow;
      throw GdpException('Request failed: $e');
    }
  }
...
/// Exception thrown by the GDP Data API client.
class GdpException implements Exception {
  final String message;

  GdpException(this.message);

  @override
  String toString() => 'GdpException: $message';
}
